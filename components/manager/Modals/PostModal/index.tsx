// src/app/(manager)/enrollment-manager/posts/_components/Modals/PostModal.tsx
"use client";

import React, { useState, useEffect } from 'react';
import { 
  X, Save, Bold, Italic, Underline, List, ListOrdered, 
  Image as ImageIcon, Link2, Quote 
} from 'lucide-react';
...
export default function PostModal({ isOpen, onClose, initialData, onSubmit }: Props) {
  ...
  // Mockup hàm xử lý Rich Text Editor
      ...
      <div className={`${styles.modalContainer} max-w-4xl`}>
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-start bg-white">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Vui lòng điền đầy đủ thông tin để xuất bản nội dung bài đăng mới.</p>
          </div>
          <button 
            type="button" 
            onClick={onClose} 
        ...
        <form id="postForm" onSubmit={handleSubmit} className={`${styles.modalBody} px-8 py-6 space-y-6`}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-1 md:col-span-2 space-y-2">
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-widest block">Tiêu đề bài đăng <span className="text-red-500">*</span></label>
              <input 
                required
                type="text"
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-widest block">Danh mục <span className="text-red-500">*</span></label>
              <select 
                required
              ...
              >
                <option disabled value="">Chọn danh mục bài đăng</option>
            ...
            {/* Status Toggle */}
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-widest block">Trạng thái xuất bản</label>
          ...
          {/* Summary */}
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-slate-600 uppercase tracking-widest block">Tóm tắt nội dung</label>
            <textarea 
          ...
          <div className="space-y-2">
            <label className="text-[11px] font-bold text-slate-600 uppercase tracking-widest block">Nội dung bài viết <span className="text-red-500">*</span></label>
            <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm focus-within:ring-2 focus-within:ring-blue-600 transition-all">
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: formData.content || 'Bắt đầu viết nội dung bài viết chuyên nghiệp tại đây...' }}
              >
              </div>
            </div>
