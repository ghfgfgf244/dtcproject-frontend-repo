...
import { SendNotificationRequest, NotificationType, NOTIFICATION_TYPE_MAP } from '@/types/notification';
import styles from '@/components/manager/Modals/modal.module.css';

interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<SendNotificationRequest>) => void;
}

const CATEGORIES = [
  { value: NotificationType.System, label: 'Thông báo hệ thống' },
  { value: NotificationType.Exam, label: 'Thông báo kỳ thi' },
  { value: NotificationType.Class, label: 'Thông báo lớp học' },
  { value: NotificationType.Registration, label: 'Đăng ký khóa học' },
  { value: NotificationType.Welcome, label: 'Chào mừng' }
];

export default function NotificationModal({ isOpen, onClose, onSubmit }: Props) {
  const [formData, setFormData] = useState<Partial<SendNotificationRequest>>({
    title: '',
    content: '',
