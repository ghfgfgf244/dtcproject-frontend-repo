...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  batchContext: { id: string; name: string }; // Cần biết đang tạo bài thi cho Đợt nào
    ...
    <div className={styles.overlay}>
      ...
      <div className={`${styles.modalContainer} ${styles.modalMaxLg}`}>
        ...
        <div className={styles.modalBody}>
          <div className="space-y-6">
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" defaultValue={initialData?.examName} className="w-full bg-slate-100 border-none rounded-lg focus:ring-2 focus:ring-blue-600 transition-all text-sm py-2.5 px-4 font-medium outline-none" placeholder="VD: Thi sát hạch Lý thuyết" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Hình thức thi</label>
                <select defaultValue={initialData?.examType || 'Theory'} className="w-full bg-slate-100 border-none rounded-lg focus:ring-2 focus:ring-blue-600 transition-all text-sm py-2.5 px-4 font-medium outline-none cursor-pointer">
              ...
              <div className="space-y-1.5">
                ...
                <input type="date" defaultValue={initialData?.examDate} className="w-full bg-slate-100 border-none rounded-lg focus:ring-2 focus:ring-blue-600 transition-all text-sm py-2 px-4 font-medium outline-none" />
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Thời lượng (Phút)</label>
              <div className="flex items-center">
                <input type="number" defaultValue={initialData?.durationMinutes} min="1" placeholder="60" className="w-full bg-slate-100 border-none rounded-l-lg focus:ring-2 focus:ring-blue-600 transition-all text-sm py-2.5 px-4 font-medium outline-none" />
        ...
        <div className="px-6 py-5 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2 text-sm font-bold text-slate-600 hover:text-slate-900 transition-colors">Hủy bỏ</button>
          <button className="bg-blue-600 text-white px-8 py-2.5 rounded-lg text-sm font-bold shadow-lg hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-2">
