...
export default async function AdminNotificationDetailPage({ 
  ...
  // Breadcrumbs dành riêng cho Admin
  ...
  if (!detailData) {
    return (
      <div className="p-6 md:p-8 flex flex-col min-h-screen bg-slate-50">
        ...
        <div className="flex flex-col items-center justify-center min-h-[50vh] bg-white rounded-xl border border-slate-200 shadow-sm">
          <h2 className="text-2xl font-bold text-red-600 mb-2">Lỗi truy xuất hệ thống</h2>
          <p className="text-slate-500">Bản ghi thông báo (ID: {currentId}) đã bị xóa hoặc không tồn tại trong Database.</p>
        </div>
      </div>
    );
  }
